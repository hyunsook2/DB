...
SELECT SALARY FROM EMPLOYEES WHERE FIRST_NAME LIKE '_____%';
...
SELECT E.EMPLOYEE_ID, D.DEPARTMENT_NAME, J.JOB_TITLE FROM EMPLOYEES E
JOIN DEPARTMENTS D ON E.DEPARTMENT_ID = D.DEPARTMENT_ID JOIN JOBS J ON E.JOB_ID = J.JOB_ID;
-- 21. 모든 사원들의 사원번호, 부서명, 직무명, 도시명을 조회하라.
SELECT E.EMPLOYEE_ID, D.DEPARTMENT_NAME, J.JOB_TITLE, L.CITY FROM EMPLOYEES E
JOIN DEPARTMENTS D ON E.DEPARTMENT_ID = D.DEPARTMENT_ID JOIN JOBS J ON E.JOB_ID = J.JOB_ID
...
SELECT  TRUNC(AVG(E.SALARY)) , D.DEPARTMENT_NAME FROM DEPARTMENTS D JOIN EMPLOYEES E 
ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
WHERE (SELECT COUNT(*) FROM EMPLOYEES E2 WHERE E2.DEPARTMENT_ID = D.DEPARTMENT_ID) < 4
GROUP BY D.DEPARTMENT_NAME;
-- 25. IT 부서의 연봉 총합을 조회하라.
SELECT SUM(SALARY) FROM EMPLOYEES
WHERE DEPARTMENT_ID = (SELECT DEPARTMENT_ID FROM DEPARTMENTS WHERE DEPARTMENT_NAME = 'IT');
-- 26. 대륙명(REGIONS)별 도시의 개수를 조회하라.
SELECT R.REGION_NAME , COUNT(1) FROM COUNTRIES C , LOCATIONS L , REGIONS R
WHERE C.COUNTRY_ID = L.COUNTRY_ID AND C.REGION_ID = R.REGION_ID
GROUP BY R.REGION_NAME;
-- 27. 도시명 별 부서의 개수를 조회하라.
SELECT L.CITY, COUNT(D.DEPARTMENT_ID) FROM LOCATIONS L
LEFT JOIN DEPARTMENTS D ON L.LOCATION_ID = D.LOCATION_ID GROUP BY L.CITY;
...
SELECT * FROM EMPLOYEES WHERE HIRE_DATE<(SELECT HIRE_DATE FROM EMPLOYEES WHERE EMPLOYEE_ID=150) ORDER BY HIRE_DATE DESC;
...
SELECT DISTINCT MANAGER.* FROM EMPLOYEES MANAGER , EMPLOYEES E WHERE MANAGER.EMPLOYEE_ID = E.MANAGER_ID;
...
SELECT E.* FROM EMPLOYEES MA,EMPLOYEES E WHERE MA.EMPLOYEE_ID=E.MANAGER_ID AND E.HIRE_DATE<MA.HIRE_DATE;
-- 37. 100번 사원을 직속 상사로 두고 있는 사원들의 모든 정보를 조회하라.
SELECT * FROM EMPLOYEES WHERE MANAGER_ID=100;
-- 38. 100번 사원을 상사로 두고 있는 모든 사원들의 모든 정보를 조회하라.
SELECT * FROM EMPLOYEES WHERE MANAGER_ID=100 
OR MANAGER_ID IN(SELECT EMPLOYEE_ID FROM EMPLOYEES WHERE MANAGER_ID=100) 
OR MANAGER_ID IN(SELECT EMPLOYEE_ID FROM EMPLOYEES WHERE MANAGER_ID IN
...
SELECT LEVEL, E.* FROM EMPLOYEES E START WITH E.EMPLOYEE_ID=112 CONNECT BY PRIOR E.MANAGER_ID=E.EMPLOYEE_ID;
...
SELECT L.LOCATION_ID , COUNT(L.LOCATION_ID) FROM EMPLOYEES E , DEPARTMENTS D , LOCATIONS L 
WHERE D.DEPARTMENT_ID = E.DEPARTMENT_ID AND L.LOCATION_ID = D.LOCATION_ID GROUP BY L.LOCATION_ID ORDER BY COUNT(L.LOCATION_ID) DESC;
...
SELECT DISTINCT DEPARTMENT_NAME, ROUND(AVG(SALARY)OVER(PARTITION BY DEPARTMENT_NAME),2) 부서평균연봉,
JOB_ID,AVG(SALARY)OVER(PARTITION BY JOB_ID) 직무평균연봉 FROM (SELECT CITY FROM  (SELECT L.CITY FROM EMPLOYEES E 
JOIN DEPARTMENTS D USING(DEPARTMENT_ID) JOIN LOCATIONS L USING(LOCATION_ID) GROUP BY L.CITY ORDER BY COUNT(*) DESC)
...
SELECT LAST_NAME , SALARY FROM EMPLOYEES WHERE 12000 < SALARY;
-- 51. 사원번호가 176 인 사람의 LAST_NAME 과 부서 번호를 조회한다.
SELECT LAST_NAME, DEPARTMENT_ID FROM EMPLOYEES WHERE EMPLOYEE_ID IN(176);
-- 52. 연봉이 5000 에서 12000의 범위 이외인 사람들의 LAST_NAME 및 연봉을 조회힌다.
SELECT LAST_NAME , SALARY FROM EMPLOYEES WHERE 5000> SALARY OR 12000<SALARY;
...
-- 54. 20 번 및 50 번 부서에서 근무하는 모든 사원들의 LAST_NAME 및 부서 번호를 알파벳순으로 조회한다.
...
SELECT * FROM EMPLOYEES WHERE SALARY NOT IN(2500,3500,7000) AND JOB_ID (ST_CLERK  ,SA_REP) ;
...
SELECT E.LAST_NAME,D.DEPARTMENT_NAME,D.DEPARTMENT_ID FROM EMPLOYEES E JOIN DEPARTMENTS D ON D.DEPARTMENT_ID=E.DEPARTMENT_ID;
...
-- 76. 총 사원 수 및 '2005','2006','2007','2008' 년도 별 고용된 사원들의 총 수를 다음과 같이 조회한다.
SELECT DISTINCT COUNT(*) OVER(PARTITION BY EXTRACT(YEAR FROM HIRE_DATE)),
...
SELECT DISTINCT d.DEPARTMENT_ID, j.JOB_ID, SUM(e.SALARY) AS 연봉 FROM EMPLOYEES e
JOIN DEPARTMENTS d ON e.DEPARTMENT_ID = d.DEPARTMENT_ID
JOIN JOBS j ON e.JOB_ID = j.JOB_ID
GROUP BY d.DEPARTMENT_ID, j.JOB_ID ORDER BY DEPARTMENT_ID;
...
SELECT D.DEPARTMENT_NAME,E.FIRST_NAME,E.SALARY,NVL(E.COMMISION_PCT,0) AS COMMISSION_PCT
FROM EMPLOYEES E, DEPARTMENTS D WHERE E.DEPARTMENT_ID=D.DEPARTMENT_ID;

SELECT D.DEPARTMENT_NAME , E.FIRST_NAME , E.SALARY , E.COMMISSION_PCT FROM EMPLOYEES E
JOIN DEPARTMENTS D ON E.DEPARTMENT_ID=D.DEPARTMENT_ID WHERE E.COMMISSION_PCT IS NULL;
...
SELECT D.DEPARTMENT_NAME , J.JOB_TITLE , E.FIRST_NAME , E.HIRE_DATE FROM EMPLOYEES E 
JOIN DEPARTMENTS D ON E.DEPARTMENT_ID=D.DEPARTMENT_ID
JOIN JOBS J ON E.JOB_ID=J.JOB_ID;
